# %% [markdown]
# # Hermes · Experiment 3 — Does a reward-only world model learn to *select* what matters?
#
...
if IN_COLAB:
    ...
    STORAGE_ROOT = "/content/drive/MyDrive/hermes_exp3"
else:
    STORAGE_ROOT = os.environ.get("HERMES_OUT", os.path.abspath("hermes_exp3_runs"))
...
# |---|---|---|---|---|---|
# | `quick` | 24 px | 1 | 96 episodes | 2 | proves every cell runs; **numbers are meaningless and expected to fail the gate** |
# | `medium` | 32 px | 3 | 6000 × 60 steps | 40 | **run this first**: does every arm pass the learning gate? |
# | `full` | 48 px | 4 | 8000 × 60 steps | 60 | the headline configuration |
#
...
#
# No run time is promised here. For reference, Experiment 2 **measured** 20.8 s/epoch
# for the task-only arm and 40.4 s/epoch for the reconstruction arm on a T4 (the decoder
# doubles the cost). This notebook prints its own **measured** estimate after the first
# epoch of each arm.
#
...
PRESETS = {
    ...
    "quick": dict(img_size=24, seq_len=20, plan_T=10, n_train=96, n_l1=40, n_probe=40,
                  ...
                  epochs=2, batch=32, horizon=5, anchor_stride=2, seeds=(0,),
                  ckpt_every=1, snap_every=1, plan_h=4, cem_samples=32, cem_elites=6,
                  cem_iters=2, plan_chunk=8),
    # Gate-first run at 32 px: 3 seeds, ~1900 optimiser steps per run.
    "medium": dict(img_size=32, n_train=6000, epochs=40, ch=24, seeds=(0, 1, 2)),
    # The headline configuration: 48 px, 4 seeds, ~3700 optimiser steps per run.
    "full": dict(),
}
...
for _d in (RUN_DIR, CKPT_DIR, EVAL_DIR, RESULTS):
...
def make_split(n, T, seed, cfg):
    """Scene + behaviour-policy rollout. All numpy, a few MB, no images.
    ...
    R = reward_np(P, active_goal(goals, sc["sel"])[:, None], cfg).astype(np.float32)
    sc.update({"p": P, "v": V, "a": A, "r": R, "mode_ou": mode_ou})
    return sc
...
def leakage_check(split, cfg):
    ...
    feats = np.column_stack([
        ...
        g.reshape(len(g), 4), np.abs(a).mean(1), a.mean(1), split["mode_ou"]])
...
_t0 = time.time()
_look = make_split(2000 if SCALE != "quick" else 200, CFG.seq_len, 99, CFG)
print(f"simulated {len(_look['sel'])} behaviour episodes in {time.time()-_t0:.1f}s")
...
_speed = np.linalg.norm(_look["v"], axis=-1)
print(f"agent speed: mean {_speed.mean()*PXS:.2f} px/step, 95th pct "
      f"{np.percentile(_speed, 95)*PXS:.2f} px/step")
print(f"distractor whiteness (min RGB channel, want 0): {_look['drgb'].min(-1).mean():.3f}")
...
# %% [markdown]
# ### 2c. Look at the data before trusting anything downstream
#
# The **cyan square** marks the active goal. It is drawn on the figure only, never on
# the frames the models see. Check by eye that the agent (white), the two goals (black
# ring and plus) and the distractors (coloured) are always distinguishable.
# %%
GOAL_BOX = 5.5 * CFG.img_size / 48.0     # half-width (px) of the figure-only active-goal marker


def goal_box(ax, g_px, lw=1.2):
    ax.add_patch(plt.Rectangle((g_px[0] - GOAL_BOX, g_px[1] - GOAL_BOX), 2 * GOAL_BOX, 2 * GOAL_BOX,
                               fill=False, ec="#00e5ff", lw=lw))
...
def fig_frames_and_coverage():
    ...
    fig, axes = plt.subplots(n_show, k_show, figsize=(1.7 * k_show, 1.8 * n_show))
    for r in range(n_show):
        for c, t in enumerate(ts):
            ax = axes[r, c]
            ax.imshow(frames[r, t], interpolation="nearest")
            goal_box(ax, ga_all[r])
            ax.set_xticks([]); ax.set_yticks([]); ax.grid(False)
            if r == 0:
                ax.set_title(f"t={t}", fontsize=9)
        axes[r, 0].set_ylabel("%s active\nhue %.2f" % (("ring", "plus")[_look["sel"][r]],
                                                      _look["bg_hsv"][r, 0]), fontsize=8)
    fig.suptitle("Behaviour-policy episodes (cyan square = active goal, figure only)", y=1.0)
    fig.tight_layout()
    savefig(fig, "fig01_frames_grid.png")

    sp = _look
    d_act = np.linalg.norm(sp["p"] - active_goal(sp["goals"], sp["sel"])[:, None], axis=-1)
    d_ina = np.linalg.norm(sp["p"] - active_goal(sp["goals"], 1 - sp["sel"])[:, None], axis=-1)
    fig, axes = plt.subplots(1, 4, figsize=(17, 3.8))
    ax = axes[0]
    ax.hist2d(sp["p"][..., 0].ravel(), sp["p"][..., 1].ravel(), bins=40, cmap="viridis")
    ax.set_title("where the agent goes"); ax.set_xlabel("x"); ax.set_ylabel("y")
    ax.invert_yaxis(); ax.grid(False)
    ax = axes[1]
    bins = np.linspace(0, 1.2, 50)
    ax.hist(d_act.ravel(), bins, alpha=.6, label="to ACTIVE goal", color="#2e86ab")
    ...
    ax = axes[2]
    ax.hist(sp["r"].ravel(), 60, color="#555")
    ax.set_title("reward distribution"); ax.set_xlabel("reward")
    ax = axes[3]
    ax.hist(np.linalg.norm(sp["v"], axis=-1).ravel() * PXS, 50, color="#5f8d4e")
    ax.set_title("agent speed"); ax.set_xlabel("px / step")
    fig.tight_layout()
...
def gif_dataset(path, n=4, fps=8):
    ...
    frames = render_np(_look, idx, CFG, seed=5)
    ga_all = active_goal(_look["goals"], _look["sel"]) * PXS
    fig, axes = plt.subplots(1, n, figsize=(2.3 * n, 2.8))
    ims, txts = [], []
    for i, ax in enumerate(axes):
        ims.append(ax.imshow(frames[i, 0], interpolation="nearest"))
        goal_box(ax, ga_all[i])
    ...
    def upd(t):
        for i in range(n):
            ...
            txts[i].set_text("reward %+.2f" % _look["r"][i, t])
        sup.set_text("Offline data (behaviour policy) - step %d/%d" % (t, CFG.seq_len - 1))
        return ims + txts + [sup]
...
# * `C_scene`: an MLP on `z_t` predicts **only irrelevant** factors: distractor
#   positions, distractor hue (cos, sin) and background saturation and brightness.
#   Experiment 2's version predicted background **colour**; here that would directly
#   supervise the goal selector and hand C a boring win, so it is excluded.
...
def n_scene_targets(cfg):
    return 4 * cfg.n_distract + 2      # distractor xy, distractor hue cos/sin, bg sat + val
...
def get_batch(split_t, idx, cfg, stats, gen, noise_std=None):
    ...
    x = render(bt["p"], bt["goals"], bt["dpos"], bt["drgb"], bt["bg_rgb"], cfg,
               noise_std=noise_std, generator=gen)
    B, T = bt["p"].shape[:2]
    ang = 2 * math.pi * bt["dhue"]
    scene = torch.cat([bt["dpos"].reshape(B, T, -1),
                       torch.stack([torch.cos(ang), torch.sin(ang)], -1).reshape(B, 1, -1).expand(B, T, -1),
...
# ---------------------------------------------------------------------------
l1_split = make_split(CFG.n_l1, CFG.seq_len, L1_SEED, CFG)       # held out, shared
l1_t = to_torch(l1_split)
all_logs, all_snaps, STATS = [], [], {}
t_sweep, preflight_done = time.time(), False

for seed in CFG.seeds:
    need = [a for a in CFG.arms if not os.path.exists(ckpt_paths(seed, a)[0])]
    train_split = make_split(CFG.n_train, CFG.seq_len, 1000 * seed + 1, CFG)
    ...
    for arm in CFG.arms:
        ...
        log, snaps = train_arm(models[arm], arm, seed, train_t, l1_t, CFG, STATS[seed], fp)
        all_logs += log
...
for seed in CFG.seeds:
    for arm in CFG.arms:
        ...
        todo = [k for k in ("level1", "planning", "onpolicy") if k not in ev]
...
for seed in SEEDS:
    for arm in ARMS:
        r2_off, _ = r2_per_k(EVAL[(seed, arm)]["level1"]["pred"], EVAL[(seed, arm)]["level1"]["tgt"])
        r2_on, _ = r2_per_k(EVAL[(seed, arm)]["onpolicy"]["pred"], EVAL[(seed, arm)]["onpolicy"]["tgt"])
        for k in range(H + 1):
...
REG_TARGETS = ["agent_position", "agent_velocity", "active_goal", "agent_to_active_goal",
               ...
               "distractor_positions", "distractor_hues", "background_sat_val",
               "background_hue_within_band"]
...
def run_probes(Z, Y, train_mask, seed=0, steps=600):
    ...
    for _ in range(steps):
        o = net(xtr)
        loss = F.mse_loss(o[:, :-1], ytr) + F.binary_cross_entropy_with_logits(o[:, -1], ysl)
        opt.zero_grad(set_to_none=True)
        loss.backward()
        opt.step()
    with torch.no_grad():
...
def fig_probes(space, name):
    ...
    for ax, probe in zip(axes, ("linear", "mlp")):
        ...
        d = probe_df[(probe_df.space == space) & (probe_df.probe == probe)]
...
def fig_selectivity():
    ...
    for ax, (space, probe) in zip(axes, [("z", "linear"), ("z", "mlp"), ("h", "linear"), ("h", "mlp")]):
        ...
        for j, a in enumerate(PROBE_ARMS):
            dots_and_mean(ax, j, d[d.arm == a].selectivity_goal.values, ARM_COLOR[a], s=50)
...
def fig_onpolicy_and_shift():
    ...
    ax = axes[0, 0]
    for a in ARMS:
        g = onpol_df[onpol_df.arm == a].groupby("horizon")[["r2_behaviour_data", "r2_planner_trajectories"]].mean()
        ax.plot(g.index, g.r2_behaviour_data, "-", color=ARM_COLOR[a], lw=2, label=f"{tick_lab(a)} behaviour data")
        ax.plot(g.index, g.r2_planner_trajectories, "--", color=ARM_COLOR[a], lw=2,
                label=f"{tick_lab(a)} planner's trajectories")
    ax.axhline(0, color="#555", lw=1)
    ax.set_xlabel("k"); ax.set_ylabel("reward R²")
...
# ## 10. Verdict
#
# Written by the data, not by what we hoped. Separate questions that are easy to
# conflate are answered separately:
#
# 1. **LEVEL**: which latent plans best, and how well does each predict reward?
# 2. **RATE**: does any arm lose more over the imagination horizon?
# 3. **ROBUST**: do the seeds even agree on the sign?
# 4. **SELECTIVE RELEVANCE**: does B keep what reward needs and drop what it does not?
# 5. **CONTROL**: does forcing irrelevant content (C) cost anything; did D train?
# 6. **DISTRIBUTION SHIFT**: does a cleaner latent buy robustness?
#
# Any gap smaller than the seed-to-seed spread is reported as a tie. Arms that failed the
# learning gate are excluded from claims.
# %%
def verdict():
    L = []
    n = len(SEEDS)
    ok = [a for a in ARMS if a not in FAILED_ARMS]
    ps = plan_seed.set_index(["model", "seed"])

    def per_seed(model, col):
        return np.array([ps.loc[(model, s), col] for s in SEEDS])

    def compare(values_arm_minus_b, name, unit):
        d = np.asarray(values_arm_minus_b)
        worse = int((d > 0).sum())
        spread = d.std(ddof=1) if len(d) > 1 else float("nan")
        line = (f"{name}: mean {d.mean():+.4f} {unit}; B worse in {worse}/{len(d)} seeds; "
                f"seed-to-seed sd {spread:.4f}")
        if len(d) < 3:
            verdict_ = "too few seeds to say anything"
        elif abs(d.mean()) < spread:
            verdict_ = "SMALLER than the seed spread -> treat as a tie"
        elif worse == len(d):
            verdict_ = "B worse in every seed"
        elif worse == 0:
            verdict_ = "B better in every seed"
        else:
            verdict_ = "seeds disagree on the sign -> not robust"
        return line, verdict_

    L += ["=" * 78, f"HERMES EXPERIMENT 3 VERDICT   scale={SCALE}  seeds={n}  frames={CFG.img_size}px  "
          f"planning episodes={CFG.n_plan}", "=" * 78, ""]
    if SCALE == "quick":
    ...
    for a in ARMS:
        ...
        L.append(f"   {ARM_LABEL[a]:38s} passed in {int(g.passed.sum())}/{len(g)} seeds"
                 + ("   -> FAILED, excluded from claims" if a in FAILED_ARMS else ""))
    L.append("")

    L.append("1. LEVEL - planning return (real reward per step, mean ± sd over seeds)")
    for k in CONTROLLERS:
        L.append(f"   {ARM_LABEL[k]:38s} {BASE[k]['rewards'].mean():+.3f}")
    for a in ARMS:
        r, c, s_ = per_seed(a, "return"), per_seed(a, "chose_active"), per_seed(a, "success")
        L.append(f"   {ARM_LABEL[a]:38s} {r.mean():+.3f} ± {r.std(ddof=1) if n > 1 else 0:.3f}   "
                 f"normalised {per_seed(a, 'normalised').mean():.2f}   goal-choice {c.mean():.2f}   "
                 f"success {s_.mean():.2f}" + ("   [FAILED GATE]" if a in FAILED_ARMS else ""))
    if "B_task" in ok:
        for a in [a for a in ok if a != "B_task"]:
            d = plan_paired_df[plan_paired_df.vs == a].sort_values("seed").delta_return.values
            line, v = compare(d, f"   return({SHORT[a]}) - return(B)", "")
            L += [line, f"      -> {v}"]
    L.append("   reward prediction R² at k=1 (mean over seeds): " + "  ".join(
        f"{SHORT[a]} {deg_df[deg_df.model == a].r2_k1.mean():.3f}" for a in ARMS))
    L.append("")

    L.append(f"2. RATE - reward R² lost between k=1 and k={H} (a separate question from level)")
    for a in ARMS:
        v = deg_df[deg_df.model == a]
        L.append(f"   {ARM_LABEL[a]:38s} R² drop {v.r2_drop.mean():.3f}   MSE growth {v.mse_ratio.mean():.2f}x")
    if "B_task" in ok:
        dB = deg_df[deg_df.model == "B_task"].sort_values("seed").r2_drop.values
        for a in [a for a in ok if a != "B_task"]:
            d = dB - deg_df[deg_df.model == a].sort_values("seed").r2_drop.values   # > 0: B loses more
            line, v = compare(d, f"   drop(B) - drop({SHORT[a]})", "R²")
            L += [line.replace("B worse", "B degrades more"), f"      -> {v}"]
    L.append("")

    L.append("3. ROBUST - do the seeds agree on the sign?")
    for a in [a for a in ARMS if a != "B_task"]:
        piv = l1_paired_df[l1_paired_df.vs == a].pivot(index="horizon", columns="seed", values="delta_mse")
        unanimous = int(((piv > 0).all(axis=1) | (piv < 0).all(axis=1)).sum())
        pl = plan_paired_df[plan_paired_df.vs == a]
        L.append(f"   B vs {SHORT[a]}: level-1 sign unanimous at {unanimous}/{H + 1} horizons; "
                 f"planning: B worse in {int((pl.delta_return > 0).sum())}/{n} seeds")
    if n < 3:
        L.append(f"   !! only {n} seed(s): this says nothing about robustness.")
    L.append("")

    L.append("4. SELECTIVE RELEVANCE")
    cb = BASE["oracle_colourblind"]
    cb_choice = planning_metrics(cb, plan_scene, CFG)["chose_active"].mean()
    or_choice = planning_metrics(BASE["oracle"], plan_scene, CFG)["chose_active"].mean()
    L.append(f"   goal-choice accuracy: oracle {or_choice:.2f}, colour-blind oracle {cb_choice:.2f} "
             "(what ignoring the background looks like)")
    for a in ARMS:
        L.append(f"   {ARM_LABEL[a]:38s} goal-choice {per_seed(a, 'chose_active').mean():.2f}")
    L.append("   selectivity = R²(active goal) - R²(inactive goal), mean over seeds "
             "(init = untrained shared starting weights):")
    for space in ("z", "h"):
        for probe in ("linear", "mlp"):
            d = sel_df[(sel_df.space == space) & (sel_df.probe == probe)]
            L.append(f"     {space} {probe:6s}: " + "  ".join(
                f"{SHORT[a]} {d[d.arm == a].selectivity_goal.mean():+.2f}" for a in PROBE_ARMS))
    pz = probe_df[(probe_df.space == "z") & (probe_df.probe == "mlp")]
    for t in ("selector_bit", "active_goal", "inactive_goal", "distractor_positions",
              "distractor_hues", "background_sat_val"):
        L.append(f"   MLP probe on z, {t:22s}: " + "  ".join(
            f"{SHORT[a]} {pz[(pz.arm == a) & (pz.target == t)].score_clipped.mean():.2f}"
            for a in PROBE_ARMS))
    if {"A_recon", "B_task"} <= set(ok):
        sB = sel_df[(sel_df.arm == "B_task") & (sel_df.space == "z") & (sel_df.probe == "mlp")].sort_values("seed")
        sA = sel_df[(sel_df.arm == "A_recon") & (sel_df.space == "z") & (sel_df.probe == "mlp")].sort_values("seed")
        dis = lambda a: pz[(pz.arm == a) & (pz.target == "distractor_positions")].sort_values("seed").score_clipped.values
        crit = {
            "B picks the active goal clearly more often than a colour-blind planner (by > 0.15)":
                per_seed("B_task", "chose_active").mean() > cb_choice + 0.15,
            "B selectivity (MLP, z) > 0 in every seed": bool((sB.selectivity_goal.values > 0).all()),
            "B selectivity > A selectivity in every seed":
                bool((sB.selectivity_goal.values > sA.selectivity_goal.values).all()),
            "B carries less distractor position than A in every seed": bool((dis("B_task") < dis("A_recon")).all()),
        }
        L.append("   criteria (stated in advance; all must hold to call it feature SELECTION):")
        for k, v in crit.items():
            L.append(f"     [{'x' if v else ' '}] {k}")
        if all(crit.values()):
            L.append("   -> B keeps what reward needs and drops distractor content: selection, not blanket discarding.")
        elif not list(crit.values())[0]:
            L.append("   -> B does not reliably use the background: consistent with blanket discarding, or not learned.")
        else:
            L.append("   -> mixed: some criteria hold, others do not. Read the probe figures before claiming anything.")
    else:
        L.append("   -> cannot be assessed: A or B failed the learning gate.")
    L.append("")

    L.append("5. CONTROL")
    if {"B_task", "C_scene"} <= set(ok):
        d = plan_paired_df[plan_paired_df.vs == "C_scene"].sort_values("seed").delta_return.values
        line, v = compare(d, "   return(C) - return(B)", "")
        L += [line, f"      -> {v}   (below 0 = forcing irrelevant content costs return)"]
    else:
        L.append("   C vs B cannot be assessed: an arm failed the gate.")
    if "D_latent" in FAILED_ARMS:
        L.append("   D_latent FAILED the learning gate. Its numbers are not evidence about self-supervised objectives.")
    else:
    ...
    if CFG.run_shift:
        L.append("6. DISTRIBUTION SHIFT - drop in return vs in-distribution (mean over seeds)")
        for c in [c for c in SHIFTS if c != "in_distribution"]:
            d = shift_paired_df[shift_paired_df.condition == c]
            L.append(f"   {c:24s} " + "  ".join(f"{SHORT[a]} {d[d.arm == a]['return_drop'].mean():+.3f}" for a in ARMS))
            if {"A_recon", "B_task"} <= set(ok):
                g = d[d.arm == "A_recon"].sort_values("seed").drop_minus_B_drop.values
                line, v = compare(g, "      drop(A) - drop(B)", "")
                L.append(f"      A vs B: {v} (above 0 = B hurt more; mean {g.mean():+.4f})")
        L.append("")

    L += ["=" * 78,
          "Level and rate are different claims. A tie is a result. Failed arms are failures,",
          "not findings. Every number above comes from this run's CSVs in results/.",
          "=" * 78]
    txt = "\n".join(L)
    print(txt)
...
#
# * **Auxiliary loss weights are not tuned.** All are 1.0. A different balance could change
#   the ordering of A, C and D.
...
# * **The selector is the most salient pixel feature.** Reconstruction gets the background
#   almost for free, which favours A on selection for a reason unrelated to the hypothesis.
#   A small, low-salience cue would favour B instead. Neither variant is run here.
...
# * **The planner has no value function**, so it only sees 10 steps ahead. The oracle uses
#   the same planner, so this caps every arm equally.
# * **Probes can mislead in both directions.** Low linear R² can hide nonlinear content,
#   and a perfectly selective model may consume the selector bit inside the encoder.
...
# ============================================================================
def bundle():
    path = os.path.join(RUN_DIR, f"hermes_exp3_{RUN_NAME}_results.zip")
